...
右键项目名称进入[Options for Target 'helloworld'],在下图所示对话框中，勾选黄框、绿框中的选项，在红框内输入命令"fromelf --bin !L --output=helloworld.bin"（bin文件建议使用项目名称),完成后按ok确认。<br><br>
...
点击蓝框内的Rebuild，进行编译，最后在项目目录下生成helloworld.bin镜像文件，其他操作参考联机烧录。<br><br>
